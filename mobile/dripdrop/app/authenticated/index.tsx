  ...
  const handleLike = useCallback(
    async (postID: number) => {
      ...
      try {
        ...
        const hasLiked = feedData.some(
          (post) => post.postID === postID && post.userHasLiked
        );
        ...
        if (hasLiked) {
          ...
          setFeedData((prevFeedData) =>
            prevFeedData.map((post) =>
              post.postID === postID
                ? { ...post, userHasLiked: false, numLikes: post.numLikes - 1 }
                : post
            )
          );
          ...
          setFeedData((prevFeedData) =>
            prevFeedData.map((post) =>
              post.postID === postID
                ? { ...post, userHasLiked: true, numLikes: post.numLikes + 1 }
                : post
            )
          );
        ...
        <View style={{ flex: 1 }}>
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              const rawImageURL = item.images && item.images[0]?.imageURL;
              const isValidImageURL = rawImageURL && rawImageURL.trim() !== "";
              const imageURL = isValidImageURL
                ? `https://cdn.dripdropco.com/${rawImageURL}?format=png`
                : null;
                          ...
                          {loadingComments ? (
                            ...
                            comments.map((comment) => (
                              ...
                              >
                                <Image
                                  source={{
                                    uri: `https://cdn.dripdropco.com/${comment.profilePic !== "default" ? comment.profilePic : "profilePics/default.jpg"}?format=png`,
                                  }}
